...
data = read.table("./data/rate-trace.txt", header=T)
...
png("../results/sum.all.packets.png", width=1920, height=1080)
...
png("../results/sum.total.packets.png", width=1920, height=1080)
print(g.packets.all.aggregate)
retval <- dev.off()

sum.packets = sum(data.packets.all.aggregate$PacketRaw)

write.csv(data.packets.all.aggregate,"../results/num.packets.csv", row.names = FALSE)
